...
## 2.4: Plot catalogues --------------------------------------------------------
pdf_path <- "./output/plots/cat_plot_motivating_example.pdf"
...
## 2.5: format simulated catalogue  ---------------------------------------------
...
# 3.0 Estiamte GPD parameters above a range of thresholds ----------------------
...
## 3.2: Subset data based on each threshold ------------------------------------
...
# Extract maguitude and threshold columns (1 list item per threshold)
mags_filtered <- map(cats_filtered, function(df){df$m})

thresholds_filtered <- map(cats_filtered, function(df){df$threshold})


## 3.3 Calculate MLE point estimate at each threshold --------------------------
...
## 3.4: Calculate event threshold exceedance probs at each threshold -----------
...
## 3.5: Sample bootstrap MLE values for each threshold -------------------------
...
# Set simulation parameters and bottstrap CI quantiles

n_eval_pts <- 101           ## number of probs at which to eval distance
