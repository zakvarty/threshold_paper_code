...
sample_mles_exp_rd_stepped <- function(mle, u, step_lengths, step_values, w, to_nearest, n_sim, verbose) {
  ...
  for(i in 1:n_sim){
    ...
    ## lambda given x
    sampled_exceedances <- sum(rbinom(n = length(w), size = 1, prob = w))
    sim_Lambda <- rpois(n = 1, lambda =  sampled_exceedances)
