...
#'
#' @param sig inital parameter value for optimisation (latent gpd parameters at threshold u)
...
#'
mle_exp_rd_varu <- function(sig, u, x, v, to_nearest = 0.1, llh_val = TRUE, hessian = FALSE,...){
  ...
  stopifnot(all(x > (v - to_nearest/2 + 1e-8)))
  ...
  #format output
  if(!llh_val & !hessian){out <-  c(temp$minimum,xi_init)} else {out <- list(params = c(temp$minimum,xi_init))}
