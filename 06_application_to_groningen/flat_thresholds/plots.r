...
pdf("./Output/plots/BO_selected_thresholds.pdf", width = 7, height = 5)
  ## Index time-scale
  plot(cat, main = "BO runs with different initial sets", pch = 16, col = 'grey70', ylab = 'magnitude')
  for( i in 1:NROW(chosen_thresholds)){
    ...
    lines(cat$index, chosen_threshold, lwd = 2, col = i+1)
  }
  abline(col = 9, h = 1.45, lty = 2)
  legend("topleft",
  ...
  ## Natural time-scale
  plot(x = gron_cat$date,y = gron_cat$mag, main = "BO runs with different initial sets", pch = 16, col = 'grey70', xlab = 'date', ylab = 'magnitude')
  for( i in 1:NROW(chosen_thresholds)){
    chosen_threshold <- get_sigmoid_values(
      tau = cat$index,
      mu = chosen_thresholds$mu[i],
      ...
      v_1 = chosen_thresholds$v_1[i],
      v_2 = chosen_thresholds$v_2[i])
    lines(gron_cat$date, chosen_threshold, lwd = 2, col = i+1)
  }
  abline(col = 9, h = 1.45, lty = 2)
  legend("topleft",
         legend = 1:NROW(chosen_thresholds),
...
pdf(pdf_path, width = 7, height = 5)
    ...
    plot(cat, main = "BO runs with different initial sets,v_1 & v_2 fixed", pch = 16, col = 'grey70', ylab = 'magnitude')
    for( i in 1:NROW(top_thresholds)){
    ...
    plot(x = gron_cat$date, gron_cat$mag, main = "BO runs with different initial sets,v_1 & v_2 fixed", xlab = "date", pch = 16, col = 'grey70', ylab = 'magnitude')
    for( i in 1:NROW(top_thresholds)){
      if(top_thresholds$rank[i] == 1){
        temp_threshold <- get_sigmoid_values(
          tau = cat$index,
    ...
    abline(col = 9, h = 1.45, lty = 2)
    abline(v = as.Date("2014-01-01")) # G-netork installation starts
    abline(v = as.Date("2014-11-05")) # First G-network sensors activated
    abline(v = as.Date("2018-01-01")) # Last G-network sensors activated
    text(x = as.Date("2012-06-01"), y = 3.5, labels = "Installation \n start")
    text(x = as.Date("2016-01-01"), y = 3.5, labels = "Sensors \n active")
    text(x = as.Date("2019-01-01"), y = 3.5, labels = "All \n active",)
    legend("topleft",
           legend = paste0(unique(top_thresholds$init), " (",round(expected_exccedances[seq(1,21,by = 5)]),")"),
           col = seq_along(unique(top_thresholds$init)) + 1,
           pch = 16,
